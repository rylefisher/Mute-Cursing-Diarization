...
class AudioCensorer:
    ...
    def _apply_crossfade(
        ...
        # Get static buffer time
        buffer_ms = (
            self.tier1_buffer if tier == 1 else self.tier2_buffer
        )  # Get buffer ms
        buffer_secs = buffer_ms / 1000.0  # Convert to sec

        # Expansion from static buffer
        base_expansion_per_side = buffer_secs / 2.0
        # Calc initial adjusted times
        initial_adj_start_time = start_time - base_expansion_per_side
        initial_adj_stop_time = stop_time + base_expansion_per_side
        initial_total_duration = initial_adj_stop_time - initial_adj_start_time

        # Ensure min duration
        min_silence_duration_secs = self.min_silence_duration
        additional_expansion_needed = 0.0
        if initial_total_duration < min_silence_duration_secs:
            additional_expansion_needed = (
                min_silence_duration_secs - initial_total_duration
            )

        # Total expansion per side
        total_expansion_per_side = base_expansion_per_side + (
            additional_expansion_needed / 2.0
        )

        # Final adjusted times
        adj_start_time = max(0, start_time - total_expansion_per_side)
        adj_stop_time = stop_time + total_expansion_per_side
        # --- End Refactored Buffer Logic ---

        audio_duration_samples = audio.shape[0]
        audio_duration_secs = audio_duration_samples / sample_rate
        adj_stop_time = min(adj_stop_time, audio_duration_secs)  # Clamp stop

        # Ensure start <= stop after clamping
        adj_start_time = min(adj_start_time, adj_stop_time)
