...
from tkinter import ttk, filedialog, scrolledtext, messagebox # Added messagebox explicitly
import subprocess
import threading
import os
import tempfile
...
def check_ffmpeg():
    ...
    try:
        ...
        subprocess.run([FFMPEG_COMMAND, "-version"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, startupinfo=get_startup_info(), timeout=5)
        logger.info("FFmpeg found.")
        return True
    except FileNotFoundError:
        logger.error("FFmpeg command not found. Please install FFmpeg and ensure it's in your system's PATH.")
        return False
    except subprocess.TimeoutExpired:
         logger.error("FFmpeg command timed out. Check FFmpeg installation.")
         return False
    except subprocess.CalledProcessError as e:
        logger.error(f"FFmpeg found but '-version' failed: {e.stderr.decode(errors='ignore')}")
...
def merge_overlapping_timestamps(timestamps, buffer_s):
    ...
    current_start, current_end = buffered_timestamps[0]['start'], buffered_timestamps[0]['end']
...
class VadApp:
    ...
    def _initialize_app(self):
        ...
        if not check_ffmpeg():
            messagebox.showerror("Dependency Error", "FFmpeg not found or not working. Please install it and add to PATH. Processing is disabled.")
    ...
    def load_vad_model(self):
        ...
        try:
            ...
            torch.set_num_threads(max(1, os.cpu_count() // 2 if os.cpu_count() else 1)) # Use reasonable threads
            ...
            messagebox.showerror("Model Load Error", f"Could not load Silero VAD model: {e}")
    ...
    def _create_widgets(self):
        ...
        ttk.Button(input_frame, text="Browse...", command=self._select_input).pack(side=tk.LEFT)
        ...
        ttk.Button(output_frame, text="Browse...", command=self._select_output).pack(side=tk.LEFT)
        ...
        ttk.Label(buffer_frame, text="Speech Buffer (ms):").pack(side=tk.LEFT, padx=(0, 10))
        self.buffer_scale = ttk.Scale(buffer_frame, from_=0, to=1000, orient=tk.HORIZONTAL, variable=self.buffer_ms, length=150, command=lambda s: self.buffer_ms.set(int(float(s))))
        ...
        self.buffer_entry = ttk.Entry(buffer_frame, textvariable=self.buffer_ms, width=5, validate='key', validatecommand=validate_cmd)
        ...
        self.codec_menu = ttk.Combobox(codec_frame, textvariable=self.codec_choice, values=['libx264'], state='readonly', width=20)
        ...
        self.status_label = ttk.Label(progress_frame, text="Status: Initializing...") # Initial status
        self.status_label.pack(side=tk.LEFT, padx=(5, 10))
        self.progress_bar = ttk.Progressbar(progress_frame, orient=tk.HORIZONTAL, length=300, mode='indeterminate')
        ...
        self.log_text = scrolledtext.ScrolledText(log_frame, height=10, state=tk.DISABLED, wrap=tk.WORD) # Slightly reduced height
        self.log_text.pack(fill=tk.BOTH, expand=True)

        # --- Start Button ---
        self.start_button = ttk.Button(main_frame, text="Start Processing", command=self._start_processing, state=tk.DISABLED) # Initially disabled
    ...
    def _update_codec_options(self):
        ...
        try:
            result = subprocess.run([FFMPEG_COMMAND, '-encoders'], capture_output=True, text=True, startupinfo=get_startup_info(), timeout=10, encoding='utf-8', errors='ignore')
            ...
            if re.search(r'^\s*V.....\s*h264_qsv', encoders, re.MULTILINE):
                ...
                if not hw_available['nvidia']: preferred_codec = 'h264_qsv' # Prefer HW if no nvidia
            if re.search(r'^\s*V.....\s*hevc_qsv', encoders, re.MULTILINE):
                codecs.append('hevc_qsv')
                hw_available['intel_qsv'] = True
                if not hw_available['nvidia']: preferred_codec = 'hevc_qsv' # Prefer newer HW if no nvidia
            ...
            if platform.system() == "Darwin": # Only check on macOS
                if re.search(r'^\s*V.....\s*h264_videotoolbox', encoders, re.MULTILINE):
                    ...
                    if not hw_available['nvidia'] and not hw_available['intel_qsv']: preferred_codec = 'h264_videotoolbox'
                if re.search(r'^\s*V.....\s*hevc_videotoolbox', encoders, re.MULTILINE):
                    codecs.append('hevc_videotoolbox')
                    hw_available['apple_vt'] = True
                    if not hw_available['nvidia'] and not hw_available['intel_qsv']: preferred_codec = 'hevc_videotoolbox'
            ...
            self._log(f"Available codecs updated. Default: {self.codec_choice.get()}")
             ...
             self._log("FFmpeg '-encoders' check timed out. Using basic codecs.", level=logging.WARNING)
             self.codec_menu['values'] = ['libx264', 'libx265']
             self.codec_choice.set('libx264')
        except Exception as e:
             self._log(f"Could not reliably check for hardware encoders: {e}. Using basic codecs.", level=logging.WARNING)
             self.codec_menu['values'] = ['libx264', 'libx265']
             self.codec_choice.set('libx264')
        ...
        if filepath:
            ...
            try:
                ...
                if suggested_output_path == filepath:
                    suggested_output_name = f"{base}_edited_1{ext}" # Add suffix if clash
                    suggested_output_path = os.path.join(input_dir, suggested_output_name)
                ...
                self._log(f"Error suggesting output filename: {e}", level=logging.WARNING)
                # Fallback: clear output or set a default if suggestion fails
                self.output_file.set("") # Or set a generic default like 'output_edited.mp4'
    ...
    def _select_output(self):
        ...
        if '265' in codec or 'hevc' in codec:
            ...
            filetypes = [("MP4 Video", "*.mp4"),("MKV Video", "*.mkv"), ("MOV Video", "*.mov"), ("All files", "*.*")]
        elif '264' in codec:
            default_ext = ".mp4"
            filetypes = [("MP4 Video", "*.mp4"), ("MKV Video", "*.mkv"), ("MOV Video", "*.mov"), ("All files", "*.*")]
    ...
    def _start_processing(self):
        ...
        except (tk.TclError, ValueError): # Catch error if entry has non-numeric value during get()
             messagebox.showerror("Error", "Invalid buffer value. Please enter a whole number (milliseconds).")
             return
        ...
        if not out_path:
            ...
            return
        if os.path.normpath(in_path) == os.path.normpath(out_path): # Robust path comparison
        ...
        if out_dir and not os.path.exists(out_dir):
            try:
                ...
                self._log(f"Created output directory: {out_dir}")
                 ...
                 messagebox.showerror("Error", f"Output directory does not exist and cannot be created:\n{out_dir}\nError: {e}")
                 return
        ...
        if self.vad_model is None:
             ...
             messagebox.showerror("Error", "VAD model not ready. Please wait or check logs.")
             return
        ...
        for widget in self.input_entry.master.winfo_children():
            if isinstance(widget, ttk.Button): widget.config(state=tk.DISABLED)
        ...
        self.progress_bar.pack(fill=tk.X, expand=True, side=tk.LEFT, padx=5) # Pack it now
        ...
        self.process_thread = threading.Thread(
            ...
            args=(in_path, out_path, buffer_val / 1000.0, self.codec_choice.get(), self.temp_dir.name), # Pass buffer in seconds
            daemon=True
        )
        self.process_thread.start()

    def _process_video_thread(self, input_path, output_path, buffer_s, codec, temp_dir_path):
        ...
        try:
            ...
            extract_cmd = [
                FFMPEG_COMMAND, '-y', # Overwrite if exists in temp
                '-i', input_path,
                '-vn',          # No video
                '-acodec', 'pcm_s16le', # Standard WAV format
                '-ar', str(SAMPLING_RATE), # Required sample rate
                '-ac', '1',     # Mono audio
                '-progress', '-', '-nostats', # Output progress to stderr for potential future parsing
                audio_path
            ]
            self._log(f"FFmpeg cmd: {' '.join(extract_cmd)}")
            process = subprocess.run(extract_cmd, check=True, capture_output=True, text=True, startupinfo=get_startup_info(), encoding='utf-8', errors='replace')
            ...
            speech_timestamps_s = [{'start': ts['start'] / SAMPLING_RATE, 'end': ts['end'] / SAMPLING_RATE} for ts in speech_timestamps]

            # 3. Apply Buffer and Merge Timestamps
            self._log(f"Step 3/4: Applying {buffer_s*1000:.0f}ms buffer and merging segments...")
            self._update_status("Merging speech segments")
            # Pass the *full* user buffer here for merging/extending segments
            final_timestamps = merge_overlapping_timestamps(speech_timestamps_s, buffer_s)
            merged_segments = len(final_timestamps)
            if merged_segments < total_segments:
                self._log(f"Merged into {merged_segments} final segments after buffering.")
            else:
                 self._log(f"Found {merged_segments} final segments (no merge required).")
            # Log first few segments for verification
            for i, ts in enumerate(final_timestamps[:min(5, len(final_timestamps))]):
                 self._log(f"  Segment {i+1}: {format_time(ts['start'])} -> {format_time(ts['end'])}")
            if len(final_timestamps) > 5: self._log("  ...")
            ...
            if not final_timestamps: # Should not happen if checked earlier, but safeguard
            ...
            select_filter_parts = [f"between(t,{ts['start']:.6f},{ts['end']:.6f})" for ts in final_timestamps]
            select_filter = f"select='{'+'.join(select_filter_parts)}',setpts=N/FRAME_RATE/TB"
            aselect_filter = f"aselect='{'+'.join(select_filter_parts)}',asetpts=N/SR/TB"

            # Handle codec options
            codec_opts = ['-c:v', codec]
            # Add quality/preset flags (example for libx264/nvenc)
            if codec == 'libx264':
                codec_opts.extend(['-preset', 'medium', '-crf', '23']) # Standard quality/speed
            elif codec == 'libx265':
                codec_opts.extend(['-preset', 'medium', '-crf', '28']) # Standard x265
            elif 'nvenc' in codec:
                 # Consider 'p5' (slower, better quality) or 'p6' (faster) based on need
                 codec_opts.extend(['-preset', 'p5', '-cq', '24', '-rc', 'vbr']) # NVENC quality mode
            elif 'qsv' in codec:
                 codec_opts.extend(['-preset', 'medium', '-global_quality', '24']) # Intel QSV (preset affects speed)
            elif 'videotoolbox' in codec:
                 # Use average bitrate for VT? Quality can be inconsistent. Or higher q:v.
                 codec_opts.extend(['-profile:v', 'main', '-q:v', '75']) # Apple VT quality (adjust as needed, lower=better)


            # FFmpeg command
            ffmpeg_cmd = [
                FFMPEG_COMMAND, '-y', # Overwrite output file
                '-i', input_path,
                '-vf', select_filter,
                '-af', aselect_filter,
            ...
            ffmpeg_cmd.extend([
                ...
                '-vsync', 'vfr', # Variable Frame Rate sync, needed with setpts
                ...
                '-progress', '-', '-nostats', # Output progress to stderr
                output_path
            ])

            self._log(f"FFmpeg cmd: {' '.join(ffmpeg_cmd)}")
            # Run and capture output
            process = subprocess.run(ffmpeg_cmd, check=True, capture_output=True, text=True, startupinfo=get_startup_info(), encoding='utf-8', errors='replace')
            ...
            if process.stderr:
                ...
                if len(stderr_lines) > log_limit * 2:
                     ...
                     for i, line in enumerate(stderr_lines):
                         is_progress = line.startswith(('frame=', 'size=', 'time=', 'bitrate=', 'speed=', 'progress='))
                         if i < log_limit or i >= len(stderr_lines) - log_limit or not is_progress:
                             filtered_lines.append(f"  [FFmpeg] {line}")
                             logged_lines_count += 1
                         elif i == log_limit and logged_lines_count == log_limit: # Indicate omission only once
                             filtered_lines.append(f"  [FFmpeg] ... (omitted {len(stderr_lines) - log_limit*2} progress lines) ...")
                     for line in filtered_lines: self._log(line)

                else:
                     for line in stderr_lines: self._log(f"  [FFmpeg] {line}")
            ...
            self._log(f"--- FFmpeg Command Failed (Exit Code {e.returncode}) ---", level=logging.ERROR)
            stderr_tail = e.stderr[-1500:] if e.stderr else "(No stderr)" # Limit stderr length
            self._log(f"FFmpeg stderr (last 1500 chars):\n{stderr_tail}", level=logging.ERROR)
            self._update_status("Error (FFmpeg)")
            # Schedule messagebox in main thread
            self.root.after(0, lambda: messagebox.showerror("FFmpeg Error", f"FFmpeg failed. Check logs for details.\n\nError message tail:\n{stderr_tail}"))
            ...
            if self.temp_dir:
                try:
                    ...
                    self._log(f"Warning: Could not clean up temp directory {self.temp_dir.name}: {e}", level=logging.WARNING)
    ...
    def _reset_gui_state(self, error=False):
        ...
        for widget in self.input_entry.master.winfo_children():
            if isinstance(widget, ttk.Button): widget.config(state=tk.NORMAL)
        for widget in self.output_entry.master.winfo_children():
            if isinstance(widget, ttk.Button): widget.config(state=tk.NORMAL)
        ...
        if not error:
             ...
             self.root.after(5000, lambda: self._update_status("Idle") if self.status_label.cget("text") == "Status: Completed" else None)
        else:
...
if __name__ == "__main__":
    ...
    if not chosen_theme:
         print(f"Preferred ttk themes not available ({preferred_themes}), using default.")
